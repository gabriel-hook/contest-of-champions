...
(function(){
  ...
  CoC.algorithm["distinct"]=new function(){
    ...
    this.build=function(options){
      var i, j;
      var synergies, champions, distinct, missing, full, value, team, worst, extraSynergies,
      ...
      fillTeams: do{
        ...
        do{
          ...
          for(i=distinct.length-1; i>=0; i--){
            synergies = distinct[i].synergies;
            champions = getHeroesFromSynergies(synergies, championsMap, teams);
            if(champions.length <= size && getTeamValue(champions, synergiesMap) > 0){
              value = getTeamValue(champions, synergiesMap);
              addTeam(teams, champions);
              distinct.splice(i, 1);
            }
          }
          didSplit = false;
          var splitDistinct = [], splitIndex = -1, splitBiggest = 0;
          ...
          if(splitIndex !== -1){
            ...
            if(result.extras){
              synergies = getDistinctSynergies( getSynergies(result.extras, synergiesMap) );
              for(i in synergies)
        ...
        tooManyTeams: while(teams.list.length > wanted){
          ...
          findfit: for(var subsize=size; subsize > 1; subsize--)
            for(i=2; i<subsize; i++){
              smallTeams = teamsBySize[i];
              largeTeams = teamsBySize[subsize - i];
              for(s in smallTeams)
                for(l in largeTeams)
                  if(smallTeams[s] !== largeTeams[l]){
                    team = smallTeams[s].team.champions.concat(largeTeams[l].team.champions);
          ...
          findcombine: for(i=2; i<size; i++){
            smallTeams = teamsBySize[i];
            largeTeams = teamsBySize[size - i + 1];
            for(s in smallTeams)
              for(l in largeTeams)
                if(smallTeams[s] !== largeTeams[l]){
                  team = combineTeams(smallTeams[s].team.champions, largeTeams[l].team.champions, 
          ...
          for(i=0; i<teams.list.length; i++)
            if(teams.list[i].champions.length < size){
              value = getTeamValue(teams.list[i].champions, synergiesMap);
        ...
        for(i=0; i<teams.list.length; i++)
          while(teams.list[i].champions.length < size && extras.length > 0){
            team = teams.list[i], best = undefined;
            
            for(j=0; j<extras.length; j++){
              value = getTeamValue([extras[j]].concat(team.champions), synergiesMap);
        ...
        extraSynergies = (function(){
          synergies = getSynergies(extras, synergiesMap);
      ...
      for(i in a)
        for(j in b){
          ...
          while(t.length > size){
            ...
            if(worst)
              t.splice(worst.index, 1);
          }
          
          if(best.count === undefined || (t.length <= size && t.length > best.count)){
            best.team = t;
            best.count = t.length;
          }
        }
    ...
    function splitDistinctGroup(group, championsMap, synergiesMap, teams, size){
      ...
      var pivot = {};
      for(i=0; i<groupHeroes.length; i++){        
        var id = groupHeroes[i].id, fid = getHeroStarId(groupHeroes[i]);
        value = 0;
      ...
      */
      ...
      var synergies = [];
      for(s = 0; s <group.synergies.length; s++){
        synergy = group.synergies[s];
      ...
      if(champions.length > size){
        ...
        while(team.length < size){
          best = null;
          for(i=0; i<champions.length; i++){
            value = getTeamValue(team.concat([ champions[i] ]), synergiesMap);
    ...
    function getSynergies(champions, synergiesMap){
      var synergies = [], ids = {}, fids = {}, fid;
      
      for(var i=0; i<champions.length; i++){
        fid = getHeroStarId(champions[i]);
